  ...
  module GitRuby
    class Repository
      ...
      def get_raw_object_by_sha1(sha1o)
        ...
        # FIXME check modified time of pack and avoid double loading
...
#        puts "*#{sha1o}*"
      ...
      private
        ...
        # TODO: do not load all packs, load lazy until searched object is found
        def init_packs
          close
          @loaded_packs = []
          @packs = []
          ...
          return if !File.exists?(path)
           Dir.open(path) do |dir|
            dir.each do |entry|
              ...
              pack_path = File.join(path,entry)
              Grit.logx { "Load pack #{pack_path}"}
              pack = Grit::GitRuby::Internal::PackStorage.new(pack_path)
