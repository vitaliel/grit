  ...
  module GitRuby
    module Internal
      ...
      class PackStorage
        ...
        def open_pack
          return if @pack_file
          Grit.logx { "Open pack file #{@name}"}
          @pack_file = File.open(@name, 'rb')
        end

        def open_idx
          return if @index_file
          fname = @name[0...-4]+'idx'
          @index_file = File.open(fname, 'rb')

          Grit.logx { "Open index file:#{fname}" }
          #debugger

          # read header
          sig = @index_file.read(4)
          ver = @index_file.read(4).unpack("N")[0]
        ...
        def close
          Grit.logx { "Pack: close files" }
          @pack_file.close if @pack_file
          @index_file.close if @index_file
          @index_file = @pack_file = nil
        end
